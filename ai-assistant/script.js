document.addEventListener('DOMContentLoaded', () => {
    ...
    function searchDataset(message) {
        ...
        if (chatbotData[message.toLowerCase()]) {
            botResponse = chatbotData[message.toLowerCase()];
        } else if (normalizedIndex.has(norm)) {
            botResponse = normalizedIndex.get(norm);
        } else if (norm.length >= 2 && norm.length <= 8) {
            const hit = normalizedKeys.find(k => k.includes(norm) || norm.includes(k));
            if (hit) botResponse = normalizedIndex.get(hit);
        }

        if (!botResponse) botResponse = "I'm sorry, I don't have an answer for that. Please try rephrasing your question.";
