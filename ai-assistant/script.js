document.addEventListener('DOMContentLoaded', () => {
    ...
    // Build a normalized index for flexible matching (handles punctuation/typos like 'hai', 'helo')
    const normalizeInput = (text) => {
        if (!text) return '';
        let t = text.toLowerCase();
        // common greeting variants
        t = t.replace(/\bhais\b/g, 'hi');
        t = t.replace(/\bhai\b/g, 'hi');
        t = t.replace(/\bheyy?\b/g, 'hey');
        t = t.replace(/\bh+e+l+o+\b/g, 'hello');
        t = t.replace(/\bh+\s*l+o+\b/g, 'hello');
        // normalize contractions spacing
        t = t.replace(/[’']/g, "'");
        // drop punctuation that often varies
        t = t.replace(/[,!.?]/g, ' ');
        // collapse whitespace
    ...
    for (const key of Object.keys(chatbotData)) {
        const nk = normalizeInput(key);
        if (!normalizedIndex.has(nk)) {
            normalizedIndex.set(nk, chatbotData[key]);
            normalizedKeys.push(nk);
        }
        // Add a greeting alias for keys that start with greeting words
        if (/^(hi|hello|hey)\b/.test(nk)) {
            const alias = nk.replace(/^(hi|hello|hey)\b/, 'hi');
            if (!normalizedIndex.has(alias)) {
                normalizedIndex.set(alias, chatbotData[key]);
                normalizedKeys.push(alias);
            }
        }
    }
    ...
    function searchDataset(message) {
        ...
        // 1) Exact legacy lookup (kept for backward compatibility)
        if (chatbotData[raw]) {
            botResponse = chatbotData[raw];
        }

        // 2) Normalized exact lookup
        if (!botResponse && normalizedIndex.has(norm)) {
            botResponse = normalizedIndex.get(norm);
        }

        // 3) Substring fallback: short inputs like 'hi', 'hai', 'hello'
        if (!botResponse && norm.length <= 8 && norm.length >= 2) {
            const hit = normalizedKeys.find(k => k.includes(norm) || norm.includes(k));
            if (hit) botResponse = normalizedIndex.get(hit);
        }
